...
*Parameterizable* in the [design thesis](#introduction) means the use of [partial application](https://en.wikipedia.org/wiki/Partial_application) to define new builds, but that doesn't roll off the tongue. [rule functions](#free-rule-functions) are curried functions that take a single [request document](#rule-request-documents) parameter.
